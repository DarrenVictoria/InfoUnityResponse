...
const validateNIC = (nic) => /^[A-Za-z0-9]{11,13}$/.test(nic);
  ...
  const handleRegister = async () => {
    ...
    if (!validateNIC(nicNumber)) {
      setErrorMessage('NIC must be 12 or 13 alphanumeric characters.');
      return;
    }

    // Validate mobile number
    if (!validateMobileNumber(mobileNumber)) {
      setErrorMessage('Mobile number must be 9 digits.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-white bg-opacity-90 p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        <h3 className="text-2xl font-semibold text-center mb-6">Register as a Respondent</h3>
        {errorMessage && (
        ...
        <div className="mt-4 text-center">
          <p className="text-sm">
            ...
            <a href="/login/respondent" className="text-blue-500 hover:underline">Login</a>
          </p>
        </div>
      </div>
    </div>
